...
class WindowedMemoryManager:
    ...
    def __init__(self, window_size: int = 10, url: str = "redis://localhost:6379", ttl: int = 3600):
        ...
        self.collection = self.mongo["chat_db"]["chat_history"]
    ...
    def load_from_mongodb(self, thread_id: str):
        """Load recent conversations from MongoDB into Redis"""
        ...
        docs = list(reversed(list(docs)))  # เรียงจากเก่าไปใหม่
        
        # แปลงเป็น BaseMessage
        messages = []
        for doc in docs:
            messages.append(HumanMessage(content=doc["user_message"]))
            messages.append(AIMessage(content=doc["ai_response"]))
    ...
    def get_messages(self, thread_id: str) -> List[BaseMessage]:
        ...
        if not messages:
            messages = self.load_from_mongodb(thread_id)  # ดึงจาก MongoDB ถ้า Redis ว่าง
            if messages:
                self._save(thread_id, messages)  # cache ลง Redis ด้วย
    ...
    async for _, mode, metadata in graph.astream(
        ...
        if mode == "messages":
            ...
            if (metadata[1].get("tags", None) == ["code"]) and is_never:
                is_never = False
                yield {"graph_ploting": mongodb_output}
